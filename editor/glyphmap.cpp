...
GlyphMap
load_glyphmap(Arena *arena, const char *font_name, FT_Library freetype)
{
   GlyphMap gm = {};
   ...
   gm.width = gm.metrics.width * GM_COUNT_X;
   ...
   for (U32 i = 0; i < ascii_chars_count; ++i) {
      ...
      for (U32 py = 0; py < bm.rows; ++py) {
         for (U32 px = 0; px < bm.width; ++px) {
            U8 pixel = bm.buffer[px + py * bm.pitch];

            U32 x = sx + px;
            U32 y = sy + py;

            gm.data[x + y * gm.width] = pixel;
         }
      }
   }

   stbi_write_jpg("out.jpg", gm.width, gm.height, 1, gm.data, 100);
